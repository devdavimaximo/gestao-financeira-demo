...
import {
  ...
  Store, Users2, LogOut, Menu, X, Moon, Sun,
} from 'lucide-react';
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? 'sidebar-nav-active text-white bg-white/10 font-semibold'
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-5 pb-4 border-b border-white/[0.07]">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-brand-gold flex items-center justify-center shrink-0">
            <TrendingUp size={15} className="text-brand-navy" strokeWidth={2.5} />
          </div>
          <div>
      ...
      <div className="px-3 py-3 border-t border-white/[0.07]">
        <div className="flex items-center gap-2.5 px-2 py-2 mb-0.5 rounded-lg">
          <div className="size-7 rounded-full bg-brand-gold flex items-center justify-center text-brand-navy font-black text-[11px] shrink-0 select-none">
            {initial}
          </div>
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile hamburger */}
      <button
        className="md:hidden fixed top-4 left-4 z-50 w-9 h-9 rounded-xl bg-brand-navy dark:bg-[#09142e] text-white shadow-lg flex items-center justify-center transition-transform active:scale-95"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.aside
            className="md:hidden fixed inset-y-0 left-0 z-50 w-60 bg-brand-navy dark:bg-[#09142e] shadow-2xl"
            initial={{ x: '-100%' }}
            animate={{ x: 0 }}
            exit={{ x: '-100%' }}
      ...
      {/* Desktop sidebar */}
      <aside
        className="hidden md:flex flex-col w-56 shrink-0 bg-brand-navy dark:bg-[#09142e] min-h-screen"
        aria-label="Menu lateral"
      >
