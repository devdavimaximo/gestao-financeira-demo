    ...
    <motion.div variants={pageVariants} initial="initial" animate="animate">
      ...
      <div className="flex flex-col gap-2 mb-4 sm:hidden">
        <div className="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-amber-600">Pendente</p>
            <p className="text-[10px] text-amber-500 mt-0.5">{payables.filter(p => p.status === 'Pending').length} conta{payables.filter(p => p.status === 'Pending').length !== 1 ? 's' : ''}</p>
          </div>
          <p className="text-lg font-black tabular-nums text-amber-700">{formatCurrency(totalPending)}</p>
        </div>
        <div className="flex items-center justify-between bg-red-50 border border-red-100 rounded-xl px-4 py-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-red-600">Vencida</p>
            <p className="text-[10px] text-red-500 mt-0.5">{payables.filter(p => p.status === 'Overdue').length} conta{payables.filter(p => p.status === 'Overdue').length !== 1 ? 's' : ''}</p>
          </div>
          <p className="text-lg font-black tabular-nums text-red-700">{formatCurrency(totalOverdue)}</p>
        </div>
        <div className="flex items-center justify-between bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Pago</p>
            <p className="text-[10px] text-emerald-500 mt-0.5">{payables.filter(p => p.status === 'Paid').length} conta{payables.filter(p => p.status === 'Paid').length !== 1 ? 's' : ''}</p>
          </div>
          <p className="text-lg font-black tabular-nums text-emerald-700">{formatCurrency(totalPaid)}</p>
        </div>
      </div>
          ...
          <>
            ...
            <div className="sm:hidden divide-y divide-gray-50">
              {payables.map(item => {
                ...
                const barColors: Record<AccountPayableStatus, string> = {
                  Pending: 'bg-amber-400', Overdue: 'bg-red-500',
                  Paid: 'bg-emerald-400', Cancelled: 'bg-gray-300',
                };
                return (
                  <div key={item.id} className="flex">
                    ...
                    <div className="flex-1 min-w-0 px-3 py-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-xs shrink-0 ${isOverdue || item.status === 'Overdue' ? 'text-red-600 font-medium' : 'text-gray-400'}`}>
                        ...
                        {canEdit && (
                          <div className="flex gap-0.5 -mr-1">
                            {(item.status === 'Pending' || item.status === 'Overdue') && (
                              <>
                                <Button variant="ghost" size="icon-sm" onClick={() => openEdit(item)} className="text-gray-300 hover:text-brand-navy h-7 w-7">
                                  <Pencil size={12} />
                                </Button>
                                <Button variant="ghost" size="icon-sm" onClick={() => openPay(item)} className="text-emerald-500 hover:text-emerald-700 h-7 w-7">
                                  <CreditCard size={12} />
                                </Button>
                              </>
                            )}
                      ...
                      {item.status === 'Paid' && item.paidAmount != null && item.paidDate && (
                        <p className="text-xs text-emerald-600 mt-1.5">
                          Pago {formatCurrency(item.paidAmount)} em {formatDateOnly(item.paidDate)}
                        </p>
                      )}
                    </div>
                  </div>
